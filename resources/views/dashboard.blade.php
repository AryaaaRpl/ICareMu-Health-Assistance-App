<x-app-layout>
    <div class="space-y-8 max-w-7xl mx-auto">
        ...
        @if(auth()->user()->role === 'guru_ismuba')
            <!-- DASHBOARD GURU ISMUBA -->
            <!-- 1. Hero Banner (Emerald / Teal Card) -->
            <div class="relative overflow-hidden bg-gradient-to-r from-emerald-800 via-teal-900 to-slate-900 rounded-3xl p-6 sm:p-8 text-white shadow-xl border border-emerald-800/40">
                <div class="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
                    <div class="space-y-3 max-w-xl">
                        <div class="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-white/10 backdrop-blur-md border border-white/20 text-xs font-semibold uppercase tracking-wider text-emerald-200">
                            ...
                            Portal Asatidz & Guru ISMUBA
                        </div>
                        <h1 class="text-2xl sm:text-4xl font-extrabold tracking-tight">
                            Selamat Datang, {{ auth()->user()->jenis_kelamin === 'P' ? 'Ustadzah' : 'Ustadz' }} {{ auth()->user()->name }}
                        </h1>
                        <p class="text-emerald-100 text-sm sm:text-base leading-relaxed">
                            Kelola modul edukasi ISMUBA, konsultasi Halo Asatidz via WhatsApp, dan pantau bimbingan keislaman serta kesehatan siswa.
                        </p>
                    </div>
                    <div class="pt-2 md:pt-0">
                        <a href="{{ route('ismuba.index') }}"
                           class="w-full sm:w-auto justify-center px-6 py-3.5 bg-white hover:bg-emerald-50 text-emerald-800 font-extrabold text-sm rounded-2xl shadow-lg hover:shadow-xl transition-all inline-flex items-center gap-2 group">
                            <span>Kelola Edukasi ISMUBA</span>
            ...
            <!-- Quick Actions Grid for Guru ISMUBA -->
            <div class="flex items-center justify-between">
                <div>
                    <h2 class="text-xl font-extrabold text-slate-900">Aksi Cepat Guru ISMUBA</h2>
                    <p class="text-xs text-slate-400 font-semibold">Fitur utama layanan edukasi keislaman dan bimbingan siswa</p>
                </div>
            </div>

            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <!-- Card 1: Kelola Materi Edukasi -->
                <a href="{{ route('ismuba.index') }}"
                   class="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm hover:shadow-md hover:border-emerald-200 transition-all flex flex-col justify-between space-y-4 group">
                    <div class="flex items-center gap-4">
                        <div class="w-14 h-14 rounded-2xl bg-emerald-50 border border-emerald-100 flex items-center justify-center text-emerald-600 group-hover:scale-105 transition-transform text-2xl">
                            📚
                        </div>
                        <div>
                            <h3 class="text-base font-bold text-slate-900 group-hover:text-emerald-600 transition-colors">Modul & Artikel ISMUBA</h3>
                            <p class="text-xs text-slate-400 font-medium">Buat dan kelola materi Fiqih, Aqidah, Akhlaq, dan Tarikh</p>
                        </div>
                    </div>
                    <div class="w-full py-2.5 px-4 bg-slate-50 group-hover:bg-emerald-600 text-slate-600 group-hover:text-white text-xs font-bold rounded-xl transition-all text-center">
                        Buka Modul Edukasi &rarr;
                    </div>
                </a>

                <!-- Card 2: Halo Asatidz / Konsultasi -->
                <a href="{{ route('ismuba.index') }}"
                   class="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm hover:shadow-md hover:border-teal-200 transition-all flex flex-col justify-between space-y-4 group">
                    <div class="flex items-center gap-4">
                        <div class="w-14 h-14 rounded-2xl bg-teal-50 border border-teal-100 flex items-center justify-center text-teal-600 group-hover:scale-105 transition-transform text-2xl">
                            👳‍♂️
                        </div>
                        <div>
                            <h3 class="text-base font-bold text-slate-900 group-hover:text-teal-600 transition-colors">Halo Asatidz</h3>
                            <p class="text-xs text-slate-400 font-medium">Layanan konsultasi kesehatan & keislaman siswa</p>
                        </div>
                    </div>
                    <div class="w-full py-2.5 px-4 bg-slate-50 group-hover:bg-teal-600 text-slate-600 group-hover:text-white text-xs font-bold rounded-xl transition-all text-center">
                        Lihat Daftar Asatidz &rarr;
                    </div>
                </a>

                <!-- Card 3: Profil Saya -->
                <a href="{{ route('profile.edit') }}"
                   class="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm hover:shadow-md hover:border-indigo-200 transition-all flex flex-col justify-between space-y-4 group">
                    <div class="flex items-center gap-4">
                        <div class="w-14 h-14 rounded-2xl bg-indigo-50 border border-indigo-100 flex items-center justify-center text-indigo-600 group-hover:scale-105 transition-transform text-2xl">
                            👤
                        </div>
                        <div>
                            <h3 class="text-base font-bold text-slate-900 group-hover:text-indigo-600 transition-colors">Profil Guru & NBM</h3>
                            <p class="text-xs text-slate-400 font-medium">Perbarui NBM (Nomor Baku Muhammadiyah) & no. WA</p>
                        </div>
                    </div>
                    <div class="w-full py-2.5 px-4 bg-slate-50 group-hover:bg-indigo-600 text-slate-600 group-hover:text-white text-xs font-bold rounded-xl transition-all text-center">
                        Edit Profil &rarr;
                    </div>
                </a>
            </div>
