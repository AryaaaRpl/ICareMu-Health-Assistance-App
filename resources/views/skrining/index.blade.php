<x-app-layout>
    ...
    <div x-data="{ openModal: false, openJadwalModal: false }" class="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div class=" space-y-6">
            <!-- Page Header -->
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                ...
                <div class="flex flex-wrap items-center gap-3">
                    <button @click="openJadwalModal = true"
                        class="inline-flex items-center gap-2 px-5 py-2.5 bg-emerald-600 hover:bg-emerald-700 active:scale-[0.97] text-white text-sm font-semibold rounded-xl shadow-lg shadow-emerald-500/25 transition-all duration-200">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5"
                                d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
                        </svg>
                        Tambah Jadwal Skrining
                    </button>
            ...
            <div class="bg-white rounded-xl border border-slate-200 p-5">
                <form method="GET" action="{{ route('skrining.index') }}"
                    class="flex flex-col sm:flex-row items-end gap-4">
                    <div class="flex-1 w-full sm:w-auto space-y-1.5">
                        <label for="search" class="text-xs font-semibold text-slate-500 uppercase tracking-wider">Cari
                            Siswa</label>
                    ...
                    <div class="w-full sm:w-48 space-y-1.5">
                        <label for="kelas"
                            class="text-xs font-semibold text-slate-500 uppercase tracking-wider">Kelas</label>
                    ...
                    @if ($searchFilter || $kelasFilter)
                        <a href="{{ route('skrining.index') }}"
                            class="text-sm font-medium text-slate-400 hover:text-slate-600 transition shrink-0">
                            Reset
                        </a>
                    @endif
                </form>
                    ...
                    <table class="w-full text-left">
                        <thead>
                            <tr class="border-b border-slate-200 bg-slate-50/50">
                                <th
                                    class="py-3.5 px-4 text-xs font-semibold text-slate-500 uppercase tracking-wider w-12 text-center">
                                    No</th>
                                <th class="py-3.5 px-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">
                                    Nama Siswa</th>
                                <th class="py-3.5 px-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">
                                    Kelas</th>
                                <th class="py-3.5 px-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">
                                ...
                                <th class="py-3.5 px-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">
                                    Tanggal</th>
                                <th
                                    class="py-3.5 px-4 text-xs font-semibold text-slate-500 uppercase tracking-wider text-center">
                                    Hasil</th>
                                <th
                                    class="py-3.5 px-4 text-xs font-semibold text-slate-500 uppercase tracking-wider text-center">
                                    Aksi</th>
                        ...
                        <tbody class="divide-y divide-slate-100">
                            @forelse($skriningRecords as $index => $record)
                                @php
                                    $siswaName = $record->siswa
                                        ? $record->siswa->nama_lengkap ??
                                            ($record->siswa->name ?? 'Siswa #' . $record->siswa_id)
                                        : 'Siswa #' . $record->siswa_id;
                                ...
                                <tr class="hover:bg-slate-50 transition-all">
                                    <td class="py-4 px-4 text-center text-sm font-medium text-slate-400">
                                        {{ $index + 1 }}</td>
                                    <td class="py-4 px-4">
                                        <div class="flex items-center gap-3">
                                            <div
                                                class="w-8 h-8 rounded-lg bg-indigo-50 text-indigo-600 flex items-center justify-center font-bold text-xs shrink-0">
                                                {{ strtoupper(substr($siswaName, 0, 1)) }}
                                            </div>
                                            <div>
                                                <span
                                                    class="text-sm font-semibold text-slate-900 block">{{ $siswaName }}</span>
                                                <span
                                                    class="text-xs text-slate-400">{{ number_format((float) $record->suhu_tubuh, 1) }}°C</span>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="py-4 px-4">
                                    ...
                                    <td class="py-4 px-4 text-sm text-slate-500 whitespace-nowrap">{{ $tanggal }}
                                    </td>
                                    <td class="py-4 px-4 text-center">
                                        @if (in_array($status, ['kembali_ke_kelas', 'sehat']))
                                            <span
                                                class="inline-flex items-center gap-1 px-2.5 py-1 rounded-md text-xs font-semibold bg-emerald-50 text-emerald-700">Kembali
                                                ke Kelas</span>
                                        @elseif(in_array($status, ['istirahat_di_uks', 'observasi_uks']))
                                            <span
                                                class="inline-flex items-center gap-1 px-2.5 py-1 rounded-md text-xs font-semibold bg-amber-50 text-amber-700">Istirahat
                                                di UKS</span>
                                        @elseif(in_array($status, ['pulang']))
                                            <span
                                                class="inline-flex items-center gap-1 px-2.5 py-1 rounded-md text-xs font-semibold bg-orange-50 text-orange-700">Diizinkan
                                                Pulang</span>
                                        @elseif(in_array($status, ['rujuk_rs', 'darurat']))
                                            <span
                                                class="inline-flex items-center gap-1 px-2.5 py-1 rounded-md text-xs font-semibold bg-rose-50 text-rose-700">Rujuk
                                                RS</span>
                                        @else
                                            <span
                                                class="inline-flex items-center gap-1 px-2.5 py-1 rounded-md text-xs font-medium bg-slate-100 text-slate-600">Menunggu</span>
                                        @endif
                                    </td>
                                    <td class="py-4 px-4 text-center">
                                ...
                                <tr>
                                    <td colspan="7" class="py-16 text-center">
                                        <div class="max-w-xs mx-auto space-y-2">
                                            ...
                                            <p class="text-xs text-slate-400">Belum ada hasil skrining siswa yang
                                                tercatat.</p>
                                        </div>
                                    </td>
                                </tr>
        ...
        <div x-show="openModal" x-transition:enter="transition ease-out duration-300"
            ...
            <div @click.away="openModal = false" x-show="openModal"
                ...
                <!-- Form -->
                <form action="{{ route('skrining.peserta.store') }}" method="POST" class="p-6 space-y-5 overflow-y-auto">
                    @csrf
                        ...
                        <select name="siswa_id" id="siswa_id" required
                            ...
                            @foreach ($siswas as $siswa)
                                <option value="{{ $siswa->id }}">
                                    {{ $siswa->name ?? ($siswa->nama_lengkap ?? 'Siswa #' . $siswa->id) }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="flex items-center gap-6">
                            <label class="inline-flex items-center gap-2.5 cursor-pointer group">
                                ...
                                <span
                                    class="text-sm font-medium text-slate-700 group-hover:text-slate-900 transition">Hadir</span>
                            </label>
                            <label class="inline-flex items-center gap-2.5 cursor-pointer group">
                                <input type="radio" name="status_kehadiran" value="Tidak Hadir"
                                    class="w-4 h-4 border-slate-300 text-indigo-600 focus:ring-indigo-500/20 transition">
                                <span
                                    class="text-sm font-medium text-slate-700 group-hover:text-slate-900 transition">Tidak
                                    Hadir</span>
                            </label>
                        </div>
                    </div>
        ...
        <!-- Alpine.js Modal (Tambah Jadwal Skrining) -->
        <div x-show="openJadwalModal" x-transition:enter="transition ease-out duration-300"
            x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100"
            x-transition:leave="transition ease-in duration-200" x-transition:leave-start="opacity-100"
            x-transition:leave-end="opacity-0"
            class="absolute inset-0 z-50 overflow-y-auto bg-slate-900/40 backdrop-blur-xs flex items-center justify-center p-4"
            style="display: none;">

            <div @click.away="openJadwalModal = false" x-show="openJadwalModal"
                x-transition:enter="transition ease-out duration-300"
                x-transition:enter-start="opacity-0 scale-95 translate-y-4"
                x-transition:enter-end="opacity-100 scale-100 translate-y-0"
                x-transition:leave="transition ease-in duration-200"
                ...
                <div class="flex items-center justify-between px-6 py-5 border-b border-slate-100 bg-slate-50/50">
                    <div>
                        <h3 class="text-lg font-bold text-slate-900">Tambah Jadwal Skrining</h3>
                        <p class="text-xs text-slate-500 mt-0.5">Buat jadwal skrining kesehatan baru untuk sekolah.</p>
                    </div>
                    <button @click="openJadwalModal = false"
                        class="p-2 rounded-xl text-slate-400 hover:text-slate-600 hover:bg-slate-100 transition duration-150">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M6 18L18 6M6 6l12 12" />
                ...
                <!-- Form -->
                <form action="{{ route('skrining.jadwal.store') }}" method="POST" class="p-6 space-y-5">
                    @csrf

                    <!-- Jenis Skrining -->
                    <div>
                        <label for="jenis_skrining"
                            class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">
                            Jenis Skrining <span class="text-rose-500">*</span>
                        </label>
                        <input type="text" name="jenis_skrining" id="jenis_skrining" required
                            placeholder="Contoh: Skrining Kesehatan Mata & Gigi"
                            class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs text-slate-800 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:bg-white transition duration-150">
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="lokasi" id="lokasi" required
                            placeholder="Contoh: Ruang UKS / Aula Sekolah"
                            class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs text-slate-800 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:bg-white transition duration-150">
                    </div>

                    <!-- Modal Actions -->
                    <div class="flex items-center justify-end gap-3 pt-4 border-t border-slate-100">
                        <button type="button" @click="openJadwalModal = false"
                            class="px-5 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-semibold rounded-xl transition duration-150">
                            Batal
                        </button>
                        <button type="submit"
                            class="px-5 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-semibold rounded-xl shadow-lg shadow-emerald-500/20 transition duration-150">
                            Simpan Jadwal
                        </button>
                    </div>
                </form>
